...
const novaPostagem = async (req, res) => {
    ...
    if (!fotos || fotos.length === 0) {
        return res.status(404).json('É preciso informar pelo menos uma foto.');
    }

    try {
        const postagem = await knex('postagens').insert({ texto: texto, usuario_id: id }).returning('*');
...
const comentar = async (req, res) => {
    ...
    if (!texto) {
        return res.status(404).json('Não foi informado nenhum comentário.');
    }
    ...
    try {
        ...
        const comentario = await knex('postagem_comentarios').insert({ usuario_id: id, postagem_id: postagemId, texto: texto });

        if (!comentario) {
            return res.status(404).json('Não foi possível comentar nessa postagem.');
        }

        return res.status(200).json('Comentário foi inserido com sucesso.');
    } catch (error) {
        return res.status(400).json(error.message);
    }
}
...
const feed = async (req, res) => {
    ...
    const pag = offset ? offset : 0;

    try {
        const postagens = await knex('postagens').where('usuario_id', '!==', id).limt(10).offset(pag);
        ...
        for (const postagem of postagens) {
            // usuario
            const usuario = await knex('usuarios').where({ id: postagem.usuario.id }).select('imagem', 'username', 'verificado').first();
            ...
            // comentarios
            const comentarios = await knex('postagem_comentarios').leftJoin('usuarios', 'usuarios.id', 'postagem_comentarios.usuario_id').where({ postagem_id: postagem.id }).select('usuario.username', 'postagem_comentarios.texto');
